...
.createCohorts <- function(connection,
  ...
  pathToCsv <- system.file("cohorts", "InclusionRules.csv", package = "OT2DSI")
  ...
  counts <- merge(counts, data.frame(cohortDefinitionId = cohortsToCreate$cohortId, cohortName  = cohortsToCreate$name)) %>%
            ...
            dplyr::mutate(cohortName = stringr::str_replace(cohortName,'OT2DSI_','')) %>%
            dplyr::mutate(cohortName = stringr::str_replace_all(cohortName,'&','(')) %>%
            dplyr::mutate(cohortName = stringr::str_replace(cohortName,'%',')')) %>%
            dplyr::mutate(cohortName = stringr::str_replace_all(cohortName,'_',' '))

  ### Create output directory:
  outputFolder_cohortstats <- paste0(output_folder,"/cohortStats")
  dir.create(outputFolder_cohortstats, showWarnings = FALSE)
  write.csv(counts, file.path(outputFolder_cohortstats, "CohortCounts.csv"), row.names= FALSE)
  ...
  #
  #   fileName <- file.path(outputFolder_cohortstats, paste0(SqlRender::snakeCaseToCamelCase(tableName), ".csv"))
  #
  ...
  # If we remove from here, makes sense to remove from the SQL script as well
